...
struct FundamentalMatrixParametrization
{
    ...
    {
        const Eigen::Map<const Eigen::Matrix3<T>> F_y{y};
        const Eigen::Map<const Eigen::Matrix3<T>> F_x{x};

        Eigen::JacobiSVD<Eigen::Matrix3<T>> svd_x{
            F_x, Eigen::ComputeFullV | Eigen::ComputeFullU};
        Eigen::JacobiSVD<Eigen::Matrix3<T>> svd_y{
            F_y, Eigen::ComputeFullV | Eigen::ComputeFullU};

        // Rotation from U_x to U_y and V_x to V_y
        Eigen::Matrix3<T> R1 = svd_x.matrixU().transpose() * svd_y.matrixU();
        Eigen::Matrix3<T> R2 = svd_x.matrixV().transpose() * svd_y.matrixV();

        // Ceres expects column-major rotation matrices, matching Eigen default
        ceres::RotationMatrixToAngleAxis(R1.data(), y_minus_x);
        ceres::RotationMatrixToAngleAxis(R2.data(), y_minus_x + 3);

        // Sigma difference
        const T sigma_x = svd_x.singularValues()(1) / svd_x.singularValues()(0);
        const T sigma_y = svd_y.singularValues()(1) / svd_y.singularValues()(0);
        y_minus_x[6] = sigma_y - sigma_x;
        return true;
    }
};
    ...
    const std::vector<Feature2D2D> &corrs, Eigen::Matrix3d *F)
{
    ...
    auto *fundamental_manifold = new ceres::AutoDiffManifold<
